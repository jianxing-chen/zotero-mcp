#!/usr/bin/env bash
# Launch a dedicated Chrome session with DevTools remote debugging enabled,
# for use with zotero_upgrade_preprint_pdfs_via_browser.
#
...
#   1. Sign in to your institutional proxy / publisher (ScienceDirect, IEEE, etc.)
#   2. Pass any bot-verification page manually
#   3. Open one article and click "View PDF" once
#   4. Keep the window open while zotero_upgrade_preprint_pdfs_via_browser runs
set -euo pipefail
...
usage() {
  cat <<'USAGE'
Usage: launch_chrome_remote_debug_macos.sh [options]
...
if [[ ! -x "$chrome_binary" ]]; then
  ...
  echo "On macOS, Google Chrome is usually at:" >&2
  echo "  /Applications/Google Chrome.app/Contents/MacOS/Google Chrome" >&2
  echo "Override with --chrome-binary PATH for a different Chromium browser." >&2
  exit 1
fi
...
echo ""
echo "Next steps in the opened Chrome window:"
echo "  1. Sign in to your institutional proxy / publisher (ScienceDirect, IEEE, etc.)"
echo "  2. Pass any bot-verification page manually"
echo "  3. Open one article and click 'View PDF' once"
echo "  4. Keep this window open while zotero_upgrade_preprint_pdfs_via_browser runs"
