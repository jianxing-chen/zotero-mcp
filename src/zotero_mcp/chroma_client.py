...
class ChromaClient:
    ...
    def _create_embedding_function(self) -> EmbeddingFunction:
        ...
        for the resolution rules. The classes it builds are the same ones
        re-exported above, which are also the ones registered with ChromaDB —
        one class object per provider name, so a persisted collection always
        rebuilds through the class that wrote its config.
        """
