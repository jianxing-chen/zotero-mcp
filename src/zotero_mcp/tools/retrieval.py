    ...
    *,
    ...
    try:
        ...
        for i, item in enumerate(display_items, 1):
            ...
            if _is_standalone_attachment(data) and data.get("contentType") == "application/pdf":
...
@with_zotero_api_lock
def audit_collection_membership(limit: int | str | None = 50, *, ctx: Context) -> str:
    """
    ...
    try:
        ...
        if not unfiled:
            ...
            if unfiled_count > limit:
                output.append(f"\n*Showing {limit} of {unfiled_count}. Increase the limit parameter to see more.*")
        output.append("")

        # --- Items in Multiple Collections section ---
        output.append(f"## Items in Multiple Collections ({multi_count}, sorted by membership count)")
        if not multi:
            ...
            for item in display:
                ...
                output.append(f"- `{key}` | {title} ({date}) — in {len(colls)}: {', '.join(names)}")
            if multi_count > limit:
                output.append(f"\n*Showing {limit} of {multi_count}. Increase the limit parameter to see more.*")
