...
@mcp.tool(
    ...
    description=(
        "Replace arXiv preprint PDF files with publisher-version PDF files, "
        "using the user's browser session when HTTP is WAF-blocked. USE THIS "
        "when the user says 'replace PDF', 'replace arXiv with publisher/"
        "official version', '换成正式版本' AND mentions a browser/campus "
        "login. Downloads the actual PDF FILE — does NOT just update metadata. "
        "\n\n"
        "For each item: (1) try ADS PUB_PDF via HTTP; (2) if blocked by "
        "WAF/403, fetch the PDF inside the user's already-authorized "
        "Chrome/Edge browser (DevTools remote debugging). Requires the user "
        "to have launched Chrome with --remote-debugging-port and signed in "
        "to their institutional proxy/publisher. Does NOT bypass paywalls. "
        "\n\n"
        "Also use zotero_upgrade_preprint_pdfs (without _via_browser) when "
        "the user does NOT mention a browser session — that is HTTP-only. "
        "\n\n"
        "Scans preprints (metadata upgraded first) and journalArticles with "
        "an arXiv ID in Extra. After success: old PDF trashed, arXiv line "
        "removed from Extra (idempotent). "
        "Trigger modes: (a) default scan; (b) require_bibcode=True; "
        "(c) item_keys=[...] for specific items; (d) collection='name or key'; "
        "(e) collection='_unfiled'. "
        "Background task — returns task_id, poll with "
