...
@mcp.tool(
    ...
    description=(
        ...
        "items from their latest local full-text source. When a MinerU "
        "cache exists for an item (from a prior 精读 via "
        "zotero_read_pdf_pages), the reindex uses MinerU's per-page text "
        "with form-feed separators — giving accurate page numbers in "
        "search results and full-document chunking (no 20-chunk cap, so "
        "a 500-page book is fully searchable). Items without a MinerU "
        "cache fall back to pdfminer as before. The incremental "
