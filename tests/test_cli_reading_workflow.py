...
def two_page_pdf(tmp_path, monkeypatch):
    ...
    doc.save(path)
    doc.close()
    monkeypatch.setattr(read_pdf, "_get_pdf_path", lambda _key, _ctx: (path, "Paper", "ATTKEY", False))
    return path
...
class TestRenderPages:
    ...
    def test_image_reads_are_capped_at_ten_pages(self, tmp_path, monkeypatch):
        ...
        doc.save(path)
        doc.close()
        monkeypatch.setattr(read_pdf, "_get_pdf_path", lambda _k, _c: (path, "Long", "ATTKEY", False))
        with pytest.raises(read_pdf.PdfReadError) as exc:
