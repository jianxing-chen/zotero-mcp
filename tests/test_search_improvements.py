...
class TestSearchWithVariants:
    ...
    def test_finds_via_original_query(self):
        ...
        result = search_module._search_with_variants(zot, "Brewer 2011", "titleCreatorYear", 10)
        assert len(result) == 1
    ...
    def test_finds_via_normalized_variant(self):
        ...
        result = search_module._search_with_variants(zot, "Müller", "titleCreatorYear", 10)
        assert len(result) == 1
    ...
    def test_deduplicates_across_variants(self):
        ...
        result = search_module._search_with_variants(zot, "Cladder-Micus", "titleCreatorYear", 10)
    ...
    def test_forwards_item_type_and_tag(self):
        ...
        search_module._search_with_variants(zot, "test", "titleCreatorYear", 10, item_type="-note", tag=["research"])
...
class TestFallbackCascade:
    ...
    def _setup(self, monkeypatch, items_by_query=None):
        ...
        monkeypatch.setattr(_utils, "_generate_search_variants", lambda q: [q])  # No variant expansion for these tests
    ...
    def test_finds_on_first_try_no_fallback(self, monkeypatch):
        items = [
            {
                "key": "X1",
                "data": {"title": "Found", "itemType": "journalArticle", "creators": [], "date": "2020", "tags": []},
            }
        ]
    ...
    def test_fallback_note_includes_verification_guidance(self, monkeypatch):
        items = [
            {
                "key": "X4",
                "data": {"title": "Paper A", "itemType": "journalArticle", "creators": [], "date": "2020", "tags": []},
            },
            {
                "key": "X5",
                "data": {"title": "Paper B", "itemType": "journalArticle", "creators": [], "date": "2021", "tags": []},
            },
        ]
        self._setup(monkeypatch, {"Brewer": items})
...
class TestMultiWordSearch:
    ...
    def test_kcl_mindfulness_matches(self, monkeypatch):
        ...
        fake_zot = MagicMock()
        fake_zot.collections = MagicMock(
            return_value=[
...
class TestCascadeSimplification:
    ...
    def _setup(self, monkeypatch, items_by_query):
        ...
        def fake_search_with_variants(
            zot, query, qmode, limit, item_type="-attachment", tag=None, cascade_start=None, cascade_timeout=None
        ):
...
class TestVerificationGuidance:
    ...
    def test_semantic_fallback_has_stronger_warning(self, monkeypatch, tmp_path):
        ...
        fake_sem = MagicMock()
        fake_sem.search.return_value = {"results": [{"item_key": "S1", "zotero_item": sem_items[0]}]}
