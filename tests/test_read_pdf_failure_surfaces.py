...
def test_unreadable_pdf_is_an_mcp_error(read_server, monkeypatch):
    monkeypatch.setattr(
        read_pdf_tools, "_get_pdf_path", lambda _key, _ctx: ("/tmp/test.pdf", "Paper", "ATTKEY", True)
    )
...
def test_successful_read_is_not_an_mcp_error(read_server, monkeypatch):
    ...
    # This suite runs against the developer's real config file; MinerU would
    # otherwise intercept the read with a background-parse task.
    from zotero_mcp import mineru_client as _mc

    monkeypatch.setattr(_mc, "load_mineru_config", lambda: {"enabled": False})
    result = _call(read_server, {"item_key": ITEM_KEY, "start_page": 1})
