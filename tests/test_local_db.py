...
# --------------------------------------------------------------------------- #
# MinerU cache priority in _extract_fulltext_for_item
# --------------------------------------------------------------------------- #
class TestMineruCachePriority:
    """The MinerU "精读" cache should take precedence over pdfminer so the
    semantic index reuses high-quality structured text (LaTeX symbols,
    tables) when available."""

    def test_mineru_cache_wins_over_pdfminer(self, tmp_path, monkeypatch):
        """When a MinerU cache exists, it is used and pdfminer is NOT called."""
        fake_pdf = tmp_path / "fake.pdf"
        fake_pdf.touch()
        # pdfminer would return this if reached — it must NOT win.
        reader = FakeLocalZoteroReader(
            fake_text="RAW_PDFMINER_TEXT", fake_pdf_path=fake_pdf
        )

        def _fake_read_mineru_cache(_self, attachment_key):
            return "MINERU_STRUCTURED_TEXT with $E=mc^2$"

        monkeypatch.setattr(
            LocalZoteroReader, "_read_mineru_cache", _fake_read_mineru_cache
        )
        result = reader._extract_fulltext_for_item(1)
        assert result is not None
        text, source = result
        assert source == "mineru-cache"
        assert "MINERU_STRUCTURED_TEXT" in text
        assert "RAW_PDFMINER_TEXT" not in text

    def test_falls_back_to_pdfminer_when_no_mineru_cache(
        self, tmp_path, monkeypatch
    ):
        """No MinerU cache → pdfminer path used as before (no regression)."""
        fake_pdf = tmp_path / "fake.pdf"
        fake_pdf.touch()
        reader = FakeLocalZoteroReader(
            fake_text="RAW_PDFMINER_TEXT", fake_pdf_path=fake_pdf
        ...
        monkeypatch.setattr(
            LocalZoteroReader, "_read_mineru_cache", lambda _self, _key: None
        )
        result = reader._extract_fulltext_for_item(1)
        assert result is not None
        text, source = result
        assert source == "pdf"
        assert text == "RAW_PDFMINER_TEXT"

    def test_empty_mineru_cache_falls_back(self, tmp_path, monkeypatch):
        """Empty/whitespace MinerU cache → fall through to pdfminer."""
        fake_pdf = tmp_path / "fake.pdf"
        fake_pdf.touch()
        reader = FakeLocalZoteroReader(
            fake_text="RAW_PDFMINER_TEXT", fake_pdf_path=fake_pdf
        ...
        monkeypatch.setattr(
            LocalZoteroReader, "_read_mineru_cache", lambda _self, _key: ""
        )
        result = reader._extract_fulltext_for_item(1)
        assert result is not None
        text, source = result
    ...
    def test_read_mineru_cache_import_failure_returns_none(
        self, tmp_path, monkeypatch
    ):
        """If the mineru_client import fails (optional dep missing),
        _read_mineru_cache returns None and the build path degrades
        gracefully to pdfminer."""
        import builtins

        real_import = builtins.__import__

        def _fail_mineru_import(name, *args, **kwargs):
            if name == "zotero_mcp.mineru_client":
                raise ImportError("mineru not installed")
            return real_import(name, *args, **kwargs)

        monkeypatch.setattr(builtins, "__import__", _fail_mineru_import)
        reader = FakeLocalZoteroReader()
        assert reader._read_mineru_cache("ANYKEY") is None
